...
CREATE DEFINER =`root`@`localhost` PROCEDURE `move_outdate_data_15`(
  ...
  BEGIN
    SET @num := 0, @code := '';
    INSERT IGNORE INTO his_15 (`date`, `open`, `close`, high, low, volume, code, ema_short, ema_long, dif, dea, macd)
      SELECT
        `date`,
        `open`,
        `close`,
...
CREATE DEFINER =`root`@`localhost` PROCEDURE `move_outdate_data_30`(
  ...
  BEGIN
    SET @num := 0, @code := '';
    INSERT IGNORE INTO his_30 (`date`, `open`, `close`, high, low, volume, code, ema_short, ema_long, dif, dea, macd)
      SELECT
        `date`,
        `open`,
        `close`,
